...
The vocabulary is 253 words in two batches, and **their provenance differs**.

Units 1–7 (204 words) are **the user's own course list**, supplied as a markdown
table. Hebrew, pronunciation and English are exactly as they gave them; the
part-of-speech and lesson groupings were added mechanically and are the parts
worth doubting. The original table is archived unparsed at
`docs/source-vocabulary.md`.

Units 8–9 (49 words) came later from a gamified language app's word list, and
are mixed: **Hebrew and English are real, the transliterations were generated
by a model.** The user was shown a spot check (four of eight debatable) and
chose to keep them anyway — a rough guide beats a blank line, and pronunciation
is not part of word identity, so any of them can be overwritten later without
costing review history. The trap is that `parse.ts` reads a `Pronunciation`
column as `authored`, so the app itself cannot mark them as guesses; the file
comments in `80-more-food.md` and `90-object-pronouns.md` are the only record.
Don't cite a unit 8–9 transliteration as evidence of anything.
